...
This example deploys a minimal single-node K3s cluster on Zadara zCompute using Ubuntu 22.04 LTS as the base OS.

## Features

- Single control plane node (all-in-one master + worker)
- Explicit Ubuntu 22.04 LTS flavor selection
- Minimal configuration using module defaults
- K3s version 1.31.2

## Why Ubuntu?

Ubuntu 22.04 LTS is the recommended flavor for K3s clusters because:

- **Long-term support**: 5 years of security updates (until 2027)
- **Wide ecosystem**: Most Kubernetes documentation and tutorials target Ubuntu
- **Tested compatibility**: Most tested configuration with this module
- **Snap support**: Easy installation of additional tools if needed

## When to Use This Example

Choose this example when:
- You want the most stable, well-tested configuration
- Your organization standardizes on Ubuntu
...
| <a name="input_name_prefix"></a> [name\_prefix](#input\_name\_prefix) | Prefix for resource names | `string` | `"k3s-ubuntu"` | no |
